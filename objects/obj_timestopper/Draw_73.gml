...
draw_clear(c_black);
surface_copy_part(behind_surface, 0, 0, application_surface, global.scale*(-150+floor(global.player.x) - camera_get_view_x(view_camera[0])),global.scale*(-150+ceil(global.player.centery) - camera_get_view_y(view_camera[0])) , 300*global.scale,300*global.scale);
...
shader_set(shd_invert);
draw_surface_ext(behind_surface, floor(global.player.x-150), ceil(global.player.centery-150),1/global.scale,1/global.scale,0,c_white,1);
shader_reset();